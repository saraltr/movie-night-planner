...
function playTrailer(trailerId) {
  ...
  // Create the trailer iframe
  const trailerIframe = document.createElement("iframe");
  trailerIframe.src = `https://www.youtube.com/embed/${trailerId}`;
  trailerIframe.allowFullscreen = true;

  // Find the existing section in the main element
  const existingSection = mainElement.querySelector(".bannerSection");

  // insert the trailer section and div after the existing section
  ...
  // create the join party button (not finished)
  ...
  watchPartyButton.addEventListener("click", () => {
    window.open("watch-party/index.html", "_blank");
  });

  // Append the button to the trailer section
...
export async function initialize() {
  ...
  // Access the movie properties directly
  const title = randomMovie.title;
  const rating = randomMovie.rating;
  const consensus = randomMovie.consensus;
  const synopsis = randomMovie.synopsis;
  const starring = randomMovie.starring;
  const directedBy = randomMovie.directed_by;
  // fetch the trailer ID for the randomly selected movie
  const trailerId = await fetchTrailerId(randomMovie.title);
  // play the trailer of the randomly selected movie
  playTrailer(trailerId);
  
  // creates current movie template
  ...
  movieInfoTemplate.innerHTML = `
    ...
    <p><strong>Staring:</strong> ${starring}</p>
    <p><strong>Directed By:</strong> ${directedBy}</p>
    <p><strong>Critics consensus:</strong> ${consensus}</p>
    </div>
  `;
...
function displayOnlineUsers() {
  ...
  // iterate through the user icons and create img elements
  userIcons.forEach((iconUrl) => {
    const userIconImg = document.createElement("img");
    userIconImg.src = iconUrl;
    userIconImg.alt = "user online icon";
    userIconImg.classList.add("userIcon");

    // append the user icon to the user icons div
    userIconsDiv.appendChild(userIconImg);
  });

  // Append the user icons div to the trailer section
